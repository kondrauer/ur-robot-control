...
class CharucoPoseEstimator:
    ...
    def __init__(
        ...
        squareX: int = 5,
        squareY: int = 7,
        squareLength: float = 0.035,
        squareWidth: float = 0.023,
        depth: bool = False,
        realsense: bool = True,
    ) -> None:
    ...
    def estimatePose(self, correction_z: float = 0.0, debug: bool = False):
        ...
        markerCorners, markerIds, _ = cv2.aruco.detectMarkers(
            colorArray,
            self.arucoDict,
            cameraMatrix=self.realsense.colorCameraMatrix,
            distCoeff=self.realsense.colorCameraDistortion,
        )

        # Check if markers are detected
        if markerIds is not None and len(markerIds) > 0:
            # Interpolate the charuco corners from markers
            cornerCount, corners, ids = cv2.aruco.interpolateCornersCharuco(
                markerCorners,
                markerIds,
                colorArray,
                self.charucoBoard,
                self.realsense.colorCameraMatrix,
                self.realsense.colorCameraDistortion,
            )
            ...
            if cornerCount > 0 and len(ids) > 0:
                ...
                success, rotationVectorWorldToCamera, translationVectorWorldToCamera = (
                ...
                if success:
                    colorArray = cv2.aruco.drawAxis(
                        colorArray,
                        self.realsense.colorCameraMatrix,
                        self.realsense.colorCameraDistortion,
                        rotationVectorWorldToCamera,
                        translationVectorWorldToCamera,
                        100.0,
                    )

                minCorners = int(
                    (self.boardSize[0] - 1) * (self.boardSize[1] - 1) * 0.5
                )  # as % of total
                ...
                if cornerCount >= minCorners and ids.size >= minCorners:
                    rotationMatrixWorldToCamera, _ = cv2.Rodrigues(
                        rotationVectorWorldToCamera
                    )

                    # There seems to be a systematic error with the z-coordinate, so we add a constant
                    translationVectorWorldToCamera[2] = (
                        translationVectorWorldToCamera[2] + correction_z
                    )
                    ...
                    if debug:
                        ...
                        logging.info(
                            f"Distance: {np.linalg.norm(translationVectorWorldToCamera)}"
                        )
    ...
    def getDepthToWorldTransform(self, debug: bool = False) -> RigidTransform:
        ...
        if success:
            tfWorldToCamera = RigidTransform(
                rotation=rMatWorldToCamera,
                translation=tVecWorldToCamera,
                from_frame="world",
                to_frame="realsense_color",
            )
            tfDepthToCamera = RigidTransform(
                rotation=self.realsense.depthToColorRotation,
                translation=self.realsense.depthToColorTranslation,
                from_frame="realsense_depth",
                to_frame="realsense_color",
            )
    ...
    def savePoseAsDepthToWorldTransform(
        self,
        path: str = "/media/max/gqcnn/gqcnn/data/calib/realsense/realsense_to_world.tf",
    ):
    ...
    def savePoseAsWorldToCameraTransform(
        self,
        path: str = "/media/max/gqcnn/gqcnn/data/calib/realsense/realsense_to_world.tf",
    ):
        """Saves current pose as depth to world transform for usage with gqcnn package
